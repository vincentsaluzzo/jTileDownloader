...
public class TileComparatorFactory {
    ...
    private static class SimpleComparator implements Comparator<Tile> {
        public int compare(Tile t1, Tile t2) {
            ...
            int r = new Integer(t1.getZ()).compareTo(t2.getZ());
            if( r == 0 ) {
                r = r = new Integer(t1.getX()).compareTo(t2.getX());
                if( r == 0 )
                    r = r = new Integer(t1.getY()).compareTo(t2.getY());
            }
            return r;
        }
    }
    ...
    private static class QuadComparator implements Comparator<Tile> {
        public int compare(Tile t1, Tile t2) {
            int r = new Integer(t1.getZ()).compareTo(t2.getZ());
            if( r == 0 ) {
		r = new Integer(t1.getZ() >> 3).compareTo(t2.getZ() >> 3);
                if( r == 0 ) {
                    r = new Integer(t1.getY() >> 3).compareTo(t2.getY() >> 3);
                    if( r == 0 ) {
                        r = new Integer(t1.getX()).compareTo(t2.getX());
                        if (r == 0) {
                            r = new Integer(t1.getY()).compareTo(t2.getY());
                        }
                    }
                }
            }
